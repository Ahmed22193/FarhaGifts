    ...
    for(let i=0;i<DataBase2.length;i++){
      content_BOX2+=`
       <div class="farha-boxes">
          ...
          <h2>${DataBase2[i].CONSTANT_Word}</h2>
          <img src="${DataBase2[i].image}" alt="">
          <p>${DataBase2[i].product_descreption}</p>
          <button onclick="Form_List2(${i})" class="buy-btn" id="ADHA-buy-btn-1">${DataBase2[i].Admin_price} LE</button>
       </div>
      `
    }
    document.getElementById("BOX2").innerHTML = content_BOX2;
}
showdata2();
...
function Form_List2(i) {
  // جمع البيانات المطلوبة من مصفوفة (DataBase1)
  const selectedData = {
    CONSTANT_Word: DataBase2[i].CONSTANT_Word,
    Admin_price: DataBase2[i].Admin_price,
    Num_of_box_:i
