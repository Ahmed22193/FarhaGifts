...
/* ---------- show data in data base 5 --------- */
function showdata5(){
    let content_BOX5 ='';
    for(let i=0;i<DataBase5.length;i++){
      content_BOX5+=`
       <div class="farha-boxes">
          ...
          <h2>${DataBase5[i].CONSTANT_Word}</h2>
          <img src="${DataBase5[i].image}" alt="">
          <p>${DataBase5[i].product_descreption}</p>
          <button onclick="Form_List5(${i})" class="buy-btn" id="ADHA-buy-btn-1">${DataBase5[i].Admin_price} LE</button>
       </div>
      `
    }
    document.getElementById("BOX5").innerHTML = content_BOX5;
}
showdata5();
  


function Form_List5(i) {
  // جمع البيانات المطلوبة من مصفوفة (DataBase1)
  const selectedData = {
    CONSTANT_Word: DataBase5[i].CONSTANT_Word,
    Admin_price: DataBase5[i].Admin_price,
    Num_of_box_:i
